...
import clip
import torch
from clip_text import class_names_ADE_150, class_names_coco, TEST
...
def write_masks_to_folder(image: np.array, masks: List[Dict[str, Any]], path: str) -> []:
    colors = random_colors(len(masks))
    mask_list = []
    for i, mask in enumerate(masks):
        mask_seg = mask['segmentation'] > 0.1

        bbox = tuple(map(int, mask['bbox']))
        x_min, y_min, width, height = bbox
        x_max = x_min + width
        y_max = y_min + height

        image_bbox = image[y_min:y_max, x_min:x_max]
        mask_bbox = mask_seg[y_min:y_max, x_min:x_max]

        masked_image = image_bbox
            # apply_mask(image_bbox, mask_bbox, colors[i]) # .copy()
        filename = f"{i}.png"
        cv2.imwrite(os.path.join(path, filename), masked_image)

        # Add the bbox of the segment
        mask_list.append(image_bbox)
    return mask_list
...
def main(args: argparse.Namespace) -> None:
    ...
    ## clip
    print("Loading clip_model")
    # clip_model, preprocess = clip.load("ViT-B/32", device=args.device)
    # cifar100 = CIFAR100(root=os.path.expanduser("~/.cache"), download=True, train=False)
    # text_features = generate_text_embeddings(cifar100.classes, ['photo of {}.'], clip_model)#['a rendering of a weird {}.'], model)
    # print(text_features)
    ...
    for t in tqdm(targets):
        ...
        masks = [mask["segmentation"] for mask in result]
        print(result[0].keys(), result[0]['bbox'])
        base = os.path.basename(t)
        base = os.path.splitext(base)[0]
        save_base = os.path.join(args.output, base)
        os.makedirs(save_base, exist_ok=True)

        mask_list = write_masks_to_folder(image_cp, result, save_base)[-2:]


        for i, mask in enumerate(mask_list):
            segment_image = torch.unsqueeze(preprocessor(Image.fromarray(mask)), 0)
            predictions = model(segment_image)
            # predictions = predictions[0]

            for prediction in predictions:
                print(f"\nTop predictions for {i}-th mask:\n")
                for id, (label, score) in enumerate(zip(prediction['labels'], prediction['scores'])):
                    print(f"#{id} {weights.meta['categories'][label.item()]} {score.item() * 100:.2f}%")

        # values, indices = torch.topk(predictions, 3)
            # print(f"\nTop predictions for {i}-th mask:\n")
            # for id, idx in enumerate(indices.squeeze()):
            #     print(f"#{id} {class_idx[str(idx.item())]} {values[0][id].item():.2f}%")
    print("Done!")
